...
function quaddu(nn)
     function quaddu_(u, p, t)
        ...
        fwx = -10^5*(p[4]*u[7]+p[7]*abs(u[7])*u[7])
        fwy = -10^5*(p[5]*u[8]+p[8]*abs(u[8])*u[8])
        fwz = -10^5*(p[6]*u[9]+p[9]*abs(u[9])*u[9])
        #println([Ix,Iy,Iz])
        #println([fwx,fwy,fwz])
...
#Flux.train!(loss_rd2, params(p_guess), take(traindata,300), opt, cb = throttle(plotfun,10))
include("data.jl")
include("../validation_tools.jl")
u0s, rds, rdns, yreals = data3()
p_guess = param([6,7,11,5,5,5,5,5,5]*10^-6)

#Flux.train!(loss_rd2, params(p_guess), take(traindata,300), opt, cb = throttle(plotfun,5))
params_guessed = zeros(9,5)
loss = zeros(1,25)
i=1
for k = 1:5
    println(k)
    idxs_test, idxs_train = partition_idxs(15,5)
    global p_guess = param([6,7,11,5,5,5,5,5,5]*10^-6)
    traindata = cycle(zip(u0s[idxs_train],rds[idxs_train],rdns[idxs_train],yreals[idxs_train]))
    testdata = cycle(zip(u0s[idxs_test],rds[idxs_test],rdns[idxs_train],yreals[idxs_test]))
    Flux.train!(loss_rd2, params(p_guess), take(traindata,300), opt)
    for j in idxs_test
